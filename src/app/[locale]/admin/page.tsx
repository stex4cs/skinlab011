  ...
  return (
    <div className="p-4 md:p-8">
      ...
      >
        ...
        >
          <h2 className="font-heading text-base md:text-lg" style={{ color: TEXT }}>
            {t("recentBookings")}
          </h2>
        </div>
        ...
        {loading ? (
          ...
          <>
                  ...
                  {bookings.map((b, i) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <div className="font-medium text-sm" style={{ color: TEXT }}>{b.client_name}</div>
                        <div className="text-xs" style={{ color: TEXT_MUTED }}>{b.client_email}</div>
                      </td>
                      <td className="px-5 py-3 text-sm" style={{ color: "rgba(255,255,255,0.65)" }}>{b.treatment_name}</td>
                      <td className="px-5 py-3 text-sm" style={{ color: "rgba(255,255,255,0.65)" }}>{b.booking_date}</td>
                      <td className="px-5 py-3 text-sm" style={{ color: "rgba(255,255,255,0.65)" }}>{b.booking_time?.slice(0, 5)}</td>
                      <td className="px-5 py-3">
                        <span
                          ...
                          {b.status === "pending" && (
                            <>
                              <button
                                ...
                                className="px-3 py-1 text-xs rounded-lg border-none cursor-pointer font-medium"
            ...
            {/* Mobile cards */}
            <div className="md:hidden divide-y" style={{ borderColor: CARD_BORDER }}>
              {bookings.map((b) => (
                <div key={b.id} className="p-4" style={{ borderTop: `1px solid ${CARD_BORDER}` }}>
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="font-medium text-sm mb-0.5" style={{ color: TEXT }}>{b.client_name}</div>
                      <div className="text-xs" style={{ color: TEXT_MUTED }}>{b.client_email}</div>
                    </div>
                    <span
                  ...
                  <div className="flex gap-2">
                    {b.status === "pending" && (
                      <>
                        <button
                          onClick={() => updateStatus(b.booking_id, "confirmed")}
                          className="flex-1 py-2 text-xs rounded-lg border-none cursor-pointer font-medium"
                          style={{ background: "rgba(102,187,106,0.15)", color: "#66BB6A", border: "1px solid rgba(102,187,106,0.3)" }}
                        >
                          {t("actions.confirm")}
                        </button>
                    ...
                    {b.status === "confirmed" && (
                      <button
                        onClick={() => updateStatus(b.booking_id, "cancelled")}
                        className="py-2 px-4 text-xs rounded-lg border-none cursor-pointer font-medium"
                        style={{ background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.5)", border: "1px solid rgba(255,255,255,0.15)" }}
                      >
                        {t("actions.cancel")}
                      </button>
